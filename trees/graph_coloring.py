...
def color_graph(graph, colors):
    for node in graph:
        if node in node.neighbors:
            raise Exception("Legal coloring impossible for \
            nodes with loops! Check node {}".format(node.label))
        
        # Get all the neighbor's colors as a set 
        # and we can check for illegal coloring 
        # by fast lookups
